...
def show_admin_panel():
    ...
    st.info("💡 Create fully formatted Question Papers. Select the 'Strict Board Paper' tab to generate a perfect 80-marks Maharashtra Board paper with 'OR' options.")
    ...
    # Helper Function to get a question text by ID
    ...
    with paper_tabs[0]:
        ...
        # ---------------------------------------------------------
            ...
            st.info(f"💡 Since {board_sub} is a theory subject (like Eco/OCM/SP), a standard 80-marks Theory Pattern will be applied.")
        ...
        # --- PREVIEW & DOWNLOAD FOR BOTH PATTERNS ---
        if st.session_state.get('board_paper_generated'):
            st.markdown("### 🖨️ Board Paper Preview & Download")
            st.success("✅ 80-Marks Board Paper Generated! No editing required, ready to print.")
            
            p_tabs = st.tabs(["📄 Question Paper", "📝 Answer Key & AI Reference"])
            with p_tabs[0]:
                with st.container(border=True):
                    st.markdown(f"```text\n{st.session_state.board_paper_text}\n```")
                st.download_button("📥 Download Board Question Paper (.txt)", data=st.session_state.board_paper_text, file_name=f"Board_80_Marks_{board_sub}.txt", mime="text/plain", type="primary", use_container_width=True)

            with p_tabs[1]:
                with st.container(border=True):
                    st.markdown(f"```text\n{st.session_state.board_ans_text}\n```")
                st.download_button("📥 Download Answer Key (.txt)", data=st.session_state.board_ans_text, file_name=f"Board_80_Ans_Key_{board_sub}.txt", mime="text/plain", type="primary", use_container_width=True)
